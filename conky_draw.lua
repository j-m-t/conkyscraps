require 'cairo'
package.path = package.path .. ';' .. os.getenv("HOME") .. '/.conky/conkyscraps/?.lua'
require 'weather_conky_draw'
...
function draw_temperature_text(display, element)
  -- Draws temperature text, with alternating color based on temperature value.
  cairo_save(display)
  cairo_move_to (display,element.from.x,element.from.y)
  cairo_rotate(display,element.rotation_angle* (math.pi / 180))
